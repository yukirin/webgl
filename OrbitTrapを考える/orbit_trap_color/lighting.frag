...
float mandelbulb(const in vec3 p, inout vec3 color);
...
float mandelbulb(const in vec3 p, inout vec3 color) {
  ...
  float bailout = 2.;
  int iterations = 7;
  vec3 point1 = vec3(0.) + sin(time);
  vec3 point2 = vec3(1.) + sin(time) * .2;
  vec3 point3 = vec3(.5) + cos(time) * .5;
  color = vec3(1000000.);
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    z += p;
    color.x = min(color.x, distance(z, point1));
    color.y = min(color.y, distance(z, point2));
    color.z = min(color.z, distance(z, point3));
  }
