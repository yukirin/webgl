...
float dSphere(const in vec3 p, const in float radius);
float dSphereLeft(const in vec3 p, const in float radius);
...
float distanceFunc(in vec3 p) {
  float ds1 = dSphere(p, 1.);
  float ds2 = dSphereLeft(p, 1.);
  float df = distFuncFloor(p);
  return min(min(ds1, ds2), df);
}
...
void intersectObjects(const in Ray ray, inout Intersection intersection, const in int bounce) {
  ...
  if (abs(dSphere(p, 1.0)) < intersectDist) {
    intersection.ambient = vec3(0.1);
    intersection.diffuse = vec3(0.8, 0.2, 0.1);
    intersection.specular = vec3(0.4);
    intersection.reflectance = 0.7;
  } else if (abs(dSphereLeft(p, 1.0)) < intersectDist) {
    intersection.ambient = vec3(0.3, 0.3, 0.6) * 1.2;
    intersection.diffuse = vec3(0.2, 0.1, 0.6) * 0.9;
    intersection.specular = vec3(1.0);
    intersection.reflectance = 0.3;
  } else if (abs(distFuncFloor(p)) < intersectDist) {
    intersection.ambient = vec3(0.1, 0.1, 0.1);
    intersection.diffuse = vec3(0.4);
    intersection.specular = vec3(1.0);
