...
float distanceFunc(in vec3 p) {
  float ds1 = distFuncSphere(p - vec3(1.2, 0., 0.), 1.0);
  float ds2 = distFuncSphere(p + vec3(1.2, 0., 0.), 1.0);
...
vec3 getRayColor(const in vec3 origin, const in vec3 ray, out vec3 rPos, out vec3 normal, out bool hit) {
  ...
  vec3 light = normalize(lightDir + vec3(sin(time * 0.2), 0.0, 0.0));
  if (!hit) {
    return vec3(0.0);
  }
  ...
  vec3 color = ((baseColor * diff + vec3(spec)) * max(0.5, shadow)) + ambientColor * ao;
