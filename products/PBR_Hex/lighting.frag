...
float repHex(in vec3 p, const in float mixParam) {
  vec3 q = p;
  q.xz -= vec2(1., .6) * mixParam;
  float height = rand(vec2(floor(q.z / 1.2), floor(q.x / 2.)));
  height += sin(time * height) * .5 + .5;
  q.z = mod(q.z, 1.2) - .6;
  q.x = mod(q.x, 2.) - 1.;
  q = rotate(q, PI * .5, vec3(1, 0, 0));
  return sdHexPrism(q, vec2(.5, height), .1);
}

float dSphereLeft(vec3 p) {
  float t = sin(2. * time);
  // p -= vec3(2. * t, 0, 3. * t);
  return distFuncSphere(p - vec3(2.5, 3, 0), 1.);
}
...
float distanceFunc(in vec3 p) {
  float d1 = repHex(p, 1.);
  float d2 = repHex(p, 0.);
  float d3 = dSphereLeft(p);
  float d4 = dSphereRight(p);

  float dh = min(d1, d2);
  float ds = smoothMin(d3, d4, 3.);
  // return ds;
  return min(dh, ds);
}
...
void intersectObjects(const in Ray ray, inout Intersection intersection, const in int bounce) {
  ...
  vec3 albedo, f0;
  float metalic;
  if (abs(repHex(p, 0.)) < intersectDist) {
    albedo = vec3(.71, .1, .01);
    metalic = 0.;
    intersection.roughness = .5;
    f0 = vec3(.04, .04, .04);
  } else if (abs(repHex(p, 1.)) < intersectDist) {
    albedo = vec3(.56, .57, .58);
    metalic = 0.0;
    intersection.roughness = .25;
    f0 = vec3(.56, .57, .58);
    ...
    f0 = vec3(.05, .05, .05);
  } else if (abs(dSphereLeft(p)) < intersectDist) {
    albedo = vec3(1., .71, .29);
    metalic = 1.;
...
void calcRadiance(inout Intersection intersection, const in Ray ray, const in int bounce) {
  ...
  // fog
  intersection.color = mix(intersection.color, vec3(0.3), 1.0 - exp(-0.0001 * pow(intersection.distance, 2.5)));
  // intersection.color += emit(intersection, ray);
  return;
}
