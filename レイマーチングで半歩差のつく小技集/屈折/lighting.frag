...
vec3 schlickFresnel(const in vec3 ri, const in vec3 view, const in vec3 normal);
...
void main(void) {
  ...
  intersection.relativeRI = vec3(1.333 / 1.0003);  // Air -> Water
  ...
  vec3 color = vec3(0.0);
  vec3 rPos;
  vec3 refraction = vec3(1);
  for (int i = 0; i < 6; i++) {
    intersectObjects(ray, intersection, i);
    color += refraction * intersection.color;
    refraction *= intersection.reflectance;

    ray.inner = dot(-ray.direction, intersection.normal) >= 0. ? 0. : 1.;
    float ri = mix(1. / intersection.relativeRI.x, intersection.relativeRI.x, ray.inner);
    vec3 orientingNormal = mix(intersection.normal, -intersection.normal, ray.inner);
    ray.direction = normalize(refract(ray.direction, orientingNormal, ri));
    ray.origin = intersection.position - orientingNormal * OFFSET;

    bool isTotalReflection = (ray.direction == vec3(0));
    if (isTotalReflection) {
      ray.direction = normalize(reflect(ray.direction, orientingNormal));
      ray.origin = intersection.position + orientingNormal * OFFSET;
    }
...
float distanceFunc(in vec3 p) {
  float ds = distFuncSphere(p, .8);
  float ds2 = distFuncSphere(p - vec3(2, 0, 0), .9);
  float db = distFuncBox(p + vec3(2, 0, 0), vec3(.5, .5, .5));
  float db2 = distFuncBox(p - vec3(0, 1.5, 0), vec3(.5, .5, .5));
  float df = distFuncFloor(p);
  return min(min(min(min(ds, ds2), db), df), db2);
}
...
float distFuncBox(vec3 p, const in vec3 size) {
  vec3 q = abs(p);
  return length(max(q - size, 0.0)) - .1;
}
...
void intersectObjects(const in Ray ray, inout Intersection intersection, const in int bounce) {
  ...
  for (int i = 0; i < 256; i++) {
    dist = distanceFunc(p);
    dist = mix(abs(dist), dist, ray.inner);
  ...
  if (abs(distFuncFloor(p)) < intersectDist) {
    intersection.normal = getNormal(intersection.position);
    intersection.ambient = vec3(0.1);
    intersection.diffuse = vec3(1) * checkeredPattern(p);
    intersection.specular = vec3(.4);
    intersection.reflectance = vec3(0.2);
  ...
  intersection.normal = getNormal(intersection.position);
  intersection.ambient = vec3(0);
  intersection.diffuse = vec3(.7, .7, .7);
  intersection.specular = vec3(1);
  intersection.reflectance = vec3(.9);
...
void calcRadiance(inout Intersection intersection, const in Ray ray, const in int bounce) {
  ...
  vec3 f = schlickFresnel(intersection.relativeRI, -ray.direction, intersection.normal);
