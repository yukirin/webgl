...
void intersectObjects(const in Ray ray, inout Intersection intersection, const in int bounce) {
  ...
  for (int i = 0; i < 256; i++) {
    dist = distanceFunc(p);
    glowMatDist = min(glowMatDist, distFuncSphere(p, 1.));
...
void calcRadiance(inout Intersection intersection, const in Ray ray, const in int bounce) {
  ...
  if (!intersection.hit) {
    ...
    intersection.color += glowMat(glowMatDist);
    return;
  }
...
vec3 glowMat(float minLength) {
  float k = 1.5;
  return vec3(.1, 1, .1) * pow(minLength + 1., -k);
}