...
vec3 foldRotate2(in vec3 p, const in float s);
...
vec3 foldRotate2(in vec3 p, const in float s) {
  float a = PI / s - atan(p.z, p.y);
  float n = (PI * 2.) / s;
  a = floor(a / n) * n;
  p = rotate(p, a, vec3(1, 0, 0));
  return p;
}
...
float dSnowCrystal(in vec3 p) {
  p = foldRotate(p, time * 1.5);
  p = foldRotate2(p, time * 1.5);
  return distTree(p);
}

float distanceFunc(in vec3 p) {
  float dt = dSnowCrystal(p);
