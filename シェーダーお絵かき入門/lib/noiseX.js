...
function noiseX(oct, ofs, per) {
  ...
  this.srnd = function(x, y) {
    var corners =
        (this.rnd(x - 1, y - 1) + this.rnd(x + 1, y - 1) + this.rnd(x - 1, y + 1) + this.rnd(x + 1, y + 1)) * 0.03125;
    var sides = (this.rnd(x - 1, y) + this.rnd(x + 1, y) + this.rnd(x, y - 1) + this.rnd(x, y + 1)) * 0.0625;
  ...
  this.snoise = function(x, y, w) {
    ...
    u = x / w;
    v = y / w;
    t = this.noise(x, y) * u * v + this.noise(x, y + w) * u * (1.0 - v) + this.noise(x + w, y) * (1.0 - u) * v +
