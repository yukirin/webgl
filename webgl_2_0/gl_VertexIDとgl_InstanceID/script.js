...
onload = function() {
  ...
  const insPositionOffset = [];
  for (let i = 0; i < 5; i++) {
    insPositionOffset.push(i * 0.3, i * 0.3, i * 0.3);
  }

  const vaos = {};
  vaos.torus = createVAO(
      [trData.p, trData.n, trData.t, insPositionOffset], ['position,3', 'normal,3', 'texCoord,2', 'offset,3'],
      [0, 0, 0, 1], trData.i, prg);
  ...
  function renderFrame(now) {
    ...
    render(
        [1.0, 1.0, 1.0], rad, [0, 1, 0], [0, 0, 0], [textures[0]], trData.i.length,
