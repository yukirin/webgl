...
vec2 brickTile(in vec2 _st, const in float _zoom) {
  _st *= _zoom;
  float side = step(1.0, mod(time, 2.0));
  float direction = step(1.0, mod(mix(_st.y, _st.x, side), 2.0)) * 2.0 - 1.0;
  _st += mix(vec2(1.0, 0.0), vec2(0.0, 1.0), side) * 1.0 * time * direction;
  return fract(_st);
}
...
void main(void) {
  ...
  // p /= vec2(2.15, 0.65) / 1.5;
  p = brickTile(p, 12.0);
  // p = rotate2d(p, PI * time * 0.5);

  color += 1.0 - vec3(circle(p, 0.3, 0.01));
  // color = vec3(p, 0.0);
