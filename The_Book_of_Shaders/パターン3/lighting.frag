...
vec2 rotateTilePattern(in vec2 _st) {
  _st *= 2.0;

  int index = 0;
  index += int(step(1.0, mod(_st.x, 2.0)));
  index += int(step(1.0, mod(_st.y, 2.0))) * 2;

  float pattern[4] = float[](PI, PI * 0.5, PI * -0.5, 0.0);
  _st = fract(_st);

  return rotate2d(_st, pattern[index]);
}
...
void main(void) {
  ...
  p = tile(p, 3.0);
  p = rotateTilePattern(p);

  // p = tile(p, 2.0);
  // p = rotate2d(p, -PI * time * 0.25);
  // p = rotateTilePattern(p * 2.);
  // p = rotate2d(p, PI * time * 0.25);

  color += vec3(step(p.x, p.y));
