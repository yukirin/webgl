...
void main(void) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      point = 0.5 + 0.5 * cos(time + 6.2831 * point);
      ...
      // m_dist = mix(0.0, m_dist, dist);
    }
  }

  // color += m_dist;
  color += m_point.x * m_point.y;
  color += mix(0.0, 0.5, sin(m_point.x * m_point.y));
  color.gb += m_dist;
