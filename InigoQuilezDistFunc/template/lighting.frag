...
float distanceFunc(in vec3 p) {
  float sphereInterval = 3.0;
  p.xz = onRep(p.xz, sphereInterval);
  float dsp = distFuncSphere(p);
  float df = distFuncFloor(p);
  return min(df, dsp);
}

vec4 sceneColor(in vec3 p) {
  const float sphereInterval = 3.0;
  vec3 pos = p;
  p.xz = onRep(p.xz, sphereInterval);
  float dsp = distFuncSphere(p);
  float df = distFuncFloor(p);

  vec3 spColor = hsv2rgb(vec3((pos.z + pos.x) / 9.0, 1.0, 1.0));
  vec3 floorColor = vec3(0.5) * checkeredPattern(pos);

  return minVec4(vec4(spColor, dsp), vec4(floorColor, df));
}
...
vec3 getRayColor(const in vec3 origin, const in vec3 ray, out vec3 rPos, out vec3 normal, out bool hit) {
  ...
  vec3 baseColor = sceneColor(rPos).rgb;
