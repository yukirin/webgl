...
function renderWebGL() {
  ...
  var fBuffer = create_framebuffer(shadowMapSize, shadowMapSize, [], true);
  ...
  var [mMatrix, vMatrix, pMatrix, tmpMatrix, mvpMatrix, invTMatrix, invMatrix, tMatrix, dvMatrix, dpMatrix, dvpMatrix] =
      initialMatrix(11);
  ...
  m.perspective(90, shadowMapSize / shadowMapSize, 0.1, 150, dpMatrix);
  ...
  (function() {
    ...
    for (var i = 0; i < 10; i++) {
      ...
      render(
          [1, 1, 1], rad, [1, 1, 1], transPos, [], torusData.i.length, trIndex, [dvpMatrix, depthMethod, mMatrix],
          ['dvpMatrix', 'depthBuffer', 'mMatrix'], ['m4', 'i1', 'm4'], depthPrg);
    }

    linkAttribute([planeData.p], ['position'], [3], depthPrg);
    render(
        [40, 1, 40], 0, [1, 1, 1], [0, -10, 0], [], planeData.i.length, pIndex, [dvpMatrix, depthMethod, mMatrix],
        ['dvpMatrix', 'depthBuffer', 'mMatrix'], ['m4', 'i1', 'm4'], depthPrg);
    ...
    for (var i = 0; i < 10; i++) {
      ...
      transPos[1] += (3 * Math.floor(i / 5) + 5);
      render(
          [1, 1, 1], rad, [1, 1, 1], transPos, [fBuffer.t], torusData.i.length, trIndex,
    ...
    render(
        [40, 1, 40], 0, [1, 1, 1], [0, -10, 0], [fBuffer.t], planeData.i.length, pIndex,
        [mMatrix, mvpMatrix, tMatrix, dvpMatrix, invTMatrix, lightPosition, 0, depthMethod],
        ['mMatrix', 'mvpMatrix', 'tMatrix', 'dvpMatrix', 'invTMatrix', 'lightPosition', 'texture', 'depthBuffer'],
        ['m4', 'm4', 'm4', 'm4', 'm4', 'v3', 'i1', 'i1'], prg);
