...
function renderWebGL() {
	...
	(function () {
		...
		render([100, 100, 100], 0, [0, 0, 0], [0, 0, 0], [textures[0]], cubeData.i.length,
			[mMatrix, mvpMatrix, invTMatrix, 1, camPosition, false, 0],
			['mMatrix', 'mvpMatrix', 'invTMatrix', 'cubeTexture', 'eyePosition', 'reflection', 'normalMap'],
			['m4', 'm4', 'm4', 'i1', 'v3', 'i1', 'i1'], prg);

		linkAttribute([sphereData.p, sphereData.c, sphereData.n, sphereData.t], ['position', 'color', 'normal', 'textureCoord'], [3, 4, 3, 2], prg);
		gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, spIndex);
		setVPMatrix(camPosition, [0, 0, 0], camUpDirection, 45, c.width, c.height, 0.1, 500);
		render([1, 1, 1], rad, [0, -1, 0], [5, 3, 0], [textures[0]], sphereData.i.length,
			[mMatrix, mvpMatrix, invTMatrix, 1, camPosition, true, 0],
			['mMatrix', 'mvpMatrix', 'invTMatrix', 'cubeTexture', 'eyePosition', 'reflection', 'normalMap'],
			['m4', 'm4', 'm4', 'i1', 'v3', 'i1', 'i1'], prg);

		linkAttribute([torusData.p, torusData.c, torusData.n, torusData.t], ['position', 'color', 'normal', 'textureCoord'], [3, 4, 3, 2], prg);
		gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, trIndex);
		setVPMatrix(camPosition, [0, 0, 0], camUpDirection, 45, c.width, c.height, 0.1, 500);
		render([1, 1, 1], rad, [1, -1, 1], [-4, 0, 0], [textures[0]], torusData.i.length,
			[mMatrix, mvpMatrix, invTMatrix, 1, camPosition, true, 0],
			['mMatrix', 'mvpMatrix', 'invTMatrix', 'cubeTexture', 'eyePosition', 'reflection', 'normalMap'],
			['m4', 'm4', 'm4', 'i1', 'v3', 'i1', 'i1'], prg);
